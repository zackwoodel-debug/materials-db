...
============================
Builds data/step1_selections_glasses.json, data/glasses.csv (oxides_50.csv column template + materialclass + selection_key) and
data/glass_gaps.csv from scripts/glass_material_list.py. Offline: no Materials Project or PubChem entry exists for a
multicomponent glass.
  ...
  * density only where the manufacturer's page states it (PROPERTIES.density, kg/m3), cited to that datasheet; otherwise NULL;
...
def main():
    ...
    for i, mat in enumerate(MATERIALS, start=1):
        ...
        selections[mat["key"]] = dict(name=mat["name"], formula=None, source="glass_material_list.py (every listed page is its own "
                                      "dataset; primary: ambient, measured before model fits, widest covering 633 nm)", axes=axes)
        ...
        if mat["density_page"]:
            ...
            flags.append("density/SLD left NULL: no density stated by the source (a multicomponent glass has no MP entry)")
        flags.append("formula NULL: multicomponent glass (no single formula); no x-ray / neutron SLD without a composition")
        if len(axes) > 1:
    ...
    df.to_csv(DATA / "glasses.csv", index=False)
    (DATA / "step1_selections_glasses.json").write_text(json.dumps(selections, indent=1, ensure_ascii=False))
    pd.DataFrame(gaps).to_csv(DATA / "glass_gaps.csv", index=False)
    print(f"Wrote glasses.csv: {len(df)} rows; {sum(len(v['axes']) for v in selections.values())} datasets; glass_gaps.csv: {len(gaps)} rows")
