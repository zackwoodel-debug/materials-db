#!/usr/bin/env python3
"""
scripts/match_ri_info_inorganic3.py
====================================
Matches scripts/inorganic3_material_list.CANDIDATES against the local RI.info catalog (authoritative) and writes
data/inorganic3_ri_matches.json + data/step1_selections_inorganic3.json.
...
MATCHES_OUT = _ROOT / "data" / "inorganic3_ri_matches.json"
SELECTIONS_OUT = _ROOT / "data" / "step1_selections_inorganic3.json"
...
def main():
    ...
    for c in CANDIDATES:
        ...
        if c["page_polymorph"]:  # user-resolved multi-paper candidate: every listed page, labelled by its own phase
            ...
            chosen = [d for d in disp if d["page"] in c["page_polymorph"]]
            out[-1]["status"] = "SELECTED_BY_USER"
            selections[c["key"]] = dict(name=c["name"], formula=c["formula"], source="user decision: " + c["basis"],
