...
A page counts as a model fit only when the source says so explicitly:
  ...
  * its COMMENTS say the data is a fit to a (simplified) model, or was generated from a model ("Data generated from oscillator
    model"), or
...
_COMMENT = re.compile(r"\bfit of [^.]*\bmodel\b|\bsimplified model\b|\bgenerated (?:from|with|using|by)\b[^.]*\bmodel\b", re.I)
