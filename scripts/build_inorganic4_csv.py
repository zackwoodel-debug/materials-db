#!/usr/bin/env python3
"""
scripts/build_inorganic4_csv.py
===============================
Builds data/inorganic4.csv (oxides_50.csv column template + materialclass + selection_key) and data/inorganic4_gaps.csv from
data/step1_selections_inorganic4.json (run match_ri_info_inorganic4.py first). Same rules as build_chalcogenides_csv.py, whose
helpers it reuses. PubChem, Materials Project and periodictable enrichment
...
Density rules (nothing is inferred):
  ...
  4. film samples get the bulk_elemental_approximation label (none in this family: film pages are excluded by the scope rule); ambiguity
     or absence leaves density and SLD NULL, with the candidates in `flags`.
...
from match_ri_info_inorganic4 import ambient  # noqa: E402
from inorganic4_material_list import AMBIENT_STRUCTURE, CANDIDATES, DEFERRED, EXCLUDED_PAGES, OUT_OF_FAMILY  # noqa: E402
...
DATA = _ROOT / "data"
PUBCHEM_NAMES = {}  # the material name (before any parenthesis) is PubChem's name for every compound here
...
def main():
    ...
    matches = {c["key"]: c for c in json.loads((DATA / "inorganic4_ri_matches.json").read_text())["candidates"]}
    rows, gaps = [], []
    with MPRester(os.environ["MP_API_KEY"]) as mpr:
        for i, c in enumerate(CANDIDATES, start=1):
            ...
            for a in other:  # a further PAPER: its n(633 nm) is evidence in flags, not an axis column
                ...
                t = a.get("temperature_c")
                flags.append(f"additional source {a['dataset_label']}: n(633 nm)={n633}" + (f", k={interp['k_633']}" if interp["k_633"] is not None else "")
                             + ("" if ambient(a) else f" at {t:g} degC (not compared: different temperature)"))
                ref = prim_n.get(a["axis"])  # compare like with like (same optical axis, ambient temperature)
                if n633 is not None and ref and ambient(a) and abs(n633 - ref) / ref > 0.02:
            ...
            if other:
                flags.append(f"{len({paper(a) for a in axes})} papers loaded as separate datasets; n_633/k_633 columns are the primary paper "
                             f"({first['dataset_label']}, widest span among ambient-temperature pages covering 633 nm)")
            if state:
    ...
    df.to_csv(DATA / "inorganic4.csv", index=False)
    ...
    pd.DataFrame(gaps).to_csv(DATA / "inorganic4_gaps.csv", index=False)
    print(f"Wrote inorganic4.csv: {len(df)} rows x {len(df.columns)} cols;  inorganic4_gaps.csv: {len(gaps)} rows")
