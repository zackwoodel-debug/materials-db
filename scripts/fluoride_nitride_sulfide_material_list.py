...
EXCLUSION_STATE = {
    "GdF3": dict(
        ...
        reason=(
            ...
            "MP spot-check). "
            "RE-VERIFIED under the corrected triage rule (see docs/PIPELINE_PRINCIPLES.md "
            "-- VN's correct entry sat at +190 meV/atom, so 'no near-degenerate "
            "alternative' is not sufficient grounds for a named exclusion on its own): "
            "re-queried mp-api for GdF3 with NO energy_above_hull filtering at all. "
            "Confirmed there is exactly ONE entry in Materials Project for this formula, "
            "full stop -- not 'none near the hull', none at any energy above hull, "
            "because there is nothing else in the candidate set to find at any window "
            "width. This is a genuinely different situation from VN (7 candidate entries, "
            "the right one just far from the hull) -- GdF3's exclusion was never a "
            "search-width problem, and widening the window changes nothing here. "
