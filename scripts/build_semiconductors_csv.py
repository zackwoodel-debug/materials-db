#!/usr/bin/env python3
"""
scripts/build_semiconductors_csv.py
====================================
Builds data/semiconductors.csv (oxides_50.csv column template + materialclass + selection_key) and data/semiconductor_gaps.csv from
data/step1_selections_semiconductors.json (run match_ri_info_semiconductors.py first). Same rules as build_chalcogenides_csv.py, whose
...
def main():
    ...
    df.to_csv(DATA / "semiconductors.csv", index=False)
    ...
    pd.DataFrame(gaps).to_csv(DATA / "semiconductor_gaps.csv", index=False)
    print(f"Wrote semiconductors.csv: {len(df)} rows x {len(df.columns)} cols;  semiconductor_gaps.csv: {len(gaps)} rows")
