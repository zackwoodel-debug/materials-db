...
single-transaction-or-rollback discipline, reusing load_oxides_db.py's
get_or_create_source/label_join/load_optical_axis/load_physical_properties
directly.
"""
...
def main():
    ...
    if n_before != 50:
        ...
        raise AssertionError(
            f"Expected exactly 50 materials already loaded (the oxide batch) before "
            f"appending batch 2, found {n_before}. Refusing to append onto an "
            f"unexpected DB state -- investigate before proceeding."
        )
    ...
    try:
        ...
        pubchem_source_id = base.get_or_create_source(
            ...
            notes="cid/smiles/inchikey/molecular_weight/CAS from PubChem PUG REST + PUG-View "
                  "CAS heading for the batch-2 fluoride/nitride/sulfide set.",
        )
        ...
        literature_source_id = base.get_or_create_source(
            conn, source_cache, "literature_density_batch2",
        ...
        bulk_approx_source_id = base.get_or_create_source(
            conn, source_cache, "bulk_approx_density_batch2",
            title="MP bulk DFT density used as a film approximation (batch-2 materials)",
            technique="MP bulk DFT density used as film approximation",
            notes="Used for TiN, VN, EuS: density_source=bulk_elemental_approximation, i.e. Materials Project's bulk-crystal "
                  "DFT density (mp_id per material in data/batch2_31.csv) standing in for a thin-film sample whose own "
                  "density is not stated. Calculated, not measured. See the flags column in data/batch2_31.csv.",
        )
        stats["sources"] += 5
    ...
    print("Committed batch 2. Row counts inserted this run:")
    ...
    print("\nTotal table row counts (oxides + batch 2):")
